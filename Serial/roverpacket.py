...
class RoverPacket(object):
	...
	def rx(self, port):
		...
		if (not port.inWaiting()): raise Exception("Error: no bytes waiting on port")
		...
		self.length = int(port.read().encode("hex"), 16)
		escaped_content = bytearray(port.read(length))
		self.message = bytearray([self.start_byte, self.addr, self.length]) + escaped_content
		self.checksum = ((~sum(self.message)) & 0xFF)
...
class BogiePacket(RoverPacket):
	...
	def from_rx(cls, port):
		new = cls(0, 0, 0)
		new.rx(port)
		if (len(new.content) != 2): raise Exception("Bogie packet contents had unexpected length of %d"%len(new))
		return new
